...
$$\forall{i} \in\{1,...,n\}; \frac{dy_i}{dt} = u_i + (u_i \frac{x_i}{K_i}) - b_iy_i + v_i \frac{y_{i}^{\beta_{i}}}{Y^{\beta_{i}} + y_{i}^{\beta_{i}}} 
...
coupled <- function(t, yini, params){
  with(as.list(c(yini, params)), {
    ...
    dy <- u + (u *((x/K))) - (b*y) + v * (y^beta/(Y^beta + y^beta)) - ((Lij %*% y) + (t(Lij) %*% y))
...
$$\forall{i} \in\{1,...,n\}; \frac{dy_i}{dt} = u_i - b_iy_i + v_i(x_i/K_i) \frac{y_{i}^{\beta_{i}}}{Y^{\beta_{i}} + y_{i}^{\beta_{i}}}
+ \sum_{j \neq i} \Delta_{ij} (y_{j} - y_{i})$$
...
coupled <- function(t, yini, params){
  with(as.list(c(yini, params)), {
    ...
    dx <- R * x * (1- x/ K ) - c *((x^a) / (1 + (x^a) )) - ((Dij %*% x) + (t(Dij) %*% x))
    dy <- u - (b*y) + v  * (x/K) * (y^beta/(Y^beta + y^beta)) - ((Lij %*% y) + (t(Lij) %*% y))
...
## slow variable 
# varying consumption term
df_pollution <- data.frame(year = seq(0, 200, 1), c = seq(0.25, 5, length.out = 201 ))
u_var <- approxfun(df_pollution)
...
$$\forall{i} \in\{1,...,n\}; \frac{dy_i}{dt} = u_i - b_iy_i + v_i \frac{y_{i}^{\beta_{i}}}{Y_i(x_i/K_i)^{\beta_{i}} + y_{i}^{\beta_{i}}} + \sum_{j \neq i} \Delta_{ij} (y_{j} - y_{i})$$
...
coupled <- function(t, yini, params){
  with(as.list(c(yini, params)), {
    ...
    dx <- R * x * (1- x/ K ) - c  *((x^a) / ( 1 + (x^a) )) - ((Dij %*% x) + (t(Dij) %*% x))
    dy <- u - (b*y) + v * (y^beta/(Y^beta + y^beta)) - ((Lij %*% y) + (t(Lij) %*% y)) * (1-(x/K))
